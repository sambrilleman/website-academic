...
draft = true
tags = ["R", "simsurv"]
title = "Simulating survival data in R"
math = true
+++

Exisiting packages in R for simulating survival (i.e. time-to-event) data can be limited in two main ways that I will discuss in this post:

In this post I am going to describe the novel features of a new package just released on CRAN, called simsurv, that can be used for simulating survival data. The novel feature of the package is that it can simulate survival data that is complex with regard to 3 main features:

1. The pacakge can simulate data from any y generally assume one of a small set of standard parametric distributions for the baseline risk of the event, for example a Weibull or a Gompertz distribution. 

2. They generally don't allow for covariates to vary over time.

time-varying 

For example, if we wish to simulate survival data under an assumed proportional hazards model, we often have to assume some parametric distribution for the baseline hazard function; for example, a Weibull or Gompertz distribution. A more flexible option would be if we were able to specify our own, user-defined function for the baseline hazard. For example, we may wish to assume a flexible baseline hazard that is defined using restricted cubic splines. 

ver wished to simulate survival (i.e. time-to-event) data in R, but found that standard parametric distributions weren't flexible enough? A new package just released on CRAN named simsurv allows the user to simulate data under a proportional hazards regression model with a user-specified hazard function.

Although a number of packages currently 
